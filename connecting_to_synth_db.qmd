...
# In CI, RETICULATE_PYTHON is set by the workflow; reticulate picks it up
# automatically. Locally on Windows, fall back to the project conda env.
local_py <- "C:/Users/corey/synth_health_data/.conda/python.exe"
if (!nzchar(Sys.getenv("RETICULATE_PYTHON")) && file.exists(local_py)) {
  use_python(local_py, required = TRUE)
}
```
